(function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!usernameVal) { showError(usernameInput, usernameError, '请输入用户名'); valid = false; }
    else if (usernameVal.length < 2) { showError(usernameInput, usernameError, '用户名至少2个字符'); valid = false; }

    if (!emailVal) { showError(emailInput, emailError, '请输入邮箱地址'); valid = false; }
    else if (!isValidEmail(emailVal)) { showError(emailInput, emailError, '邮箱格式不正确'); valid = false; }

    if (!passwordVal) { showError(passwordInput, passwordError, '请输入密码'); valid = false; }
    else if (passwordVal.length < MIN_LEN) { showError(passwordInput, passwordError, '密码长度至少' + MIN_LEN + '位'); valid = false; }

    if (!confirmVal) { showError(confirmPasswordInput, confirmPasswordError, '请确认密码'); valid = false; }
    else if (confirmVal !== passwordVal) { showError(confirmPasswordInput, confirmPasswordError, '两次密码不一致'); valid = false; }
    ...
    ApiClient.register({
      username: usernameVal, email: emailVal,
      password: passwordVal, confirmPassword: confirmVal
    }).then(function (res) {
